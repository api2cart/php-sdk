...
class OrderApi
{
    ...
    public function orderFindWithHttpInfo($start = 0, $count = 10, $customer_id = null, $customer_email = null, $order_status = null, $financial_status = null, $created_to = null, $created_from = null, $modified_to = null, $modified_from = null, $params = 'order_id,customer,totals,address,items,bundles,status', $exclude = null, string $contentType = self::contentTypes['orderFind'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\OpenAPI\Client\Model\OrderFind200Response' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\OpenAPI\Client\Model\OrderFind200Response' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\OpenAPI\Client\Model\OrderFind200Response', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\OpenAPI\Client\Model\OrderFind200Response',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
